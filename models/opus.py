...
class OPUS(MVXTwoStageDetector):
    ...
    def forward_train(self,
       ...
       # aux_loss_inputs = [voxel_semantics, mask_camera, aux_outs, img_metas]
        losses = self.pts_bbox_head.loss(*loss_inputs)
        # aux_losses = self.pts_bbox_head.loss(*aux_loss_inputs)
        # print(img_metas[0]['scene_name'])
        # if not img_metas[0]['prev_exists']:
        #      sum_dict = dict(main=sum(losses.values()).detach())       
        #      print(sum_dict)
        # merged_losses = {}
        # for key in set(losses) | set(aux_losses):  
        #     merged_losses[key] = losses.get(key, 0) + 1e-2*aux_losses.get(key, 0)
        return losses
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        if img_metas[0]['scene_name'] != self.prev_scene_name:
            self.prev_scene_name = img_metas[0]['scene_name']
            img_metas[0]['prev_exists'] = x[0].new_zeros(1)
        else:
            img_metas[0]['prev_exists'] = x[0].new_ones(1)
