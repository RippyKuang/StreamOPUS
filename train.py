...
from mmcv.runner import IterBasedRunner, build_optimizer, load_checkpoint
...
def main():
    ...
    train_loader = build_dataloader(
        ...
        shuffle=True,
        seed=0,
        shuffler_sampler=cfgs.data.shuffler_sampler,  # dict(type='DistributedGroupSampler'),
        nonshuffler_sampler=cfgs.data.nonshuffler_sampler,  # dict(type='DistributedSampler'),
    )
    ...
    runner = IterBasedRunner(
        ...
        work_dir=work_dir,
        logger=logging.root,
        max_iters=cfgs.total_epochs*cfgs.num_iters_per_epoch,
        meta=dict(),
    )

    runner.register_training_hooks(cfgs.lr_config, cfgs.optimizer_config,
                                   cfgs.checkpoint_config, cfgs.log_config,
                                   )
